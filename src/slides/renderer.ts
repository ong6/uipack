...
import { resolveNodePose, slidePalettes } from "./model";
import type {
  SlideNode,
  SlideStop,
  SlideStory,
...
/** Loaded only after mounting the optional slides entry. No browser work at module scope. */
export function createSlideScene(
  ...
  scene.add(key);
  const fill = new THREE.DirectionalLight(0x71dcb2, 1.2);
  ...
  for (const n of story.nodes) {
    ...
    const geometry =
      n.kind === "sphere"
        ? new THREE.IcosahedronGeometry(size[0] / 2, 2)
    ...
    const label = [...labels.children].find(
      (el) => (el as HTMLElement).dataset.node === n.id,
  ...
  let stop = initial,
    ...
    disposed = false,
    raf = 0,
    transition: gsap.core.Timeline | undefined;
  let clock = 0,
    lastTime = 0,
    visible = true,
    width = 1,
  ...
  function pointAt(points: THREE.Vector3[], progress: number) {
    ...
    for (let i = 0; i < lengths.length; i++) {
      if (distance <= lengths[i] || i === lengths.length - 1)
        return points[i]
          .clone()
          .lerp(points[i + 1], lengths[i] ? distance / lengths[i] : 0);
      distance -= lengths[i];
    }
    return points[0].clone();
  }
  // Exit at the box surface instead of routing through the object center.
  function anchor(node: NodeView, toward: THREE.Vector3) {
    const center = node.group.position.clone(),
      d = toward.clone().sub(center);
    const size = node.spec.size ?? [1.65, 0.6, 1.05];
    const distances = [d.x, d.y, d.z].map((v, i) =>
      Math.abs(v) > 0.0001
        ? (size[i] * node.pose.scale) / 2 / Math.abs(v)
        : Infinity,
    );
    return center
      .addScaledVector(d, Math.min(...distances, 0.45))
      .addScaledVector(d.clone().normalize(), 0.11);
  }
  function draw() {
    if (disposed) return;
    ...
    const ids = stop.labels ? new Set(stop.labels) : undefined;
    ...
    for (const n of nodes.values()) {
      ...
      if (!n.label) continue;
      const eligible =
        (ids ? ids.has(n.spec.id) : n.spec.kind !== "boundary") &&
        n.pose.opacity >= 0.3 &&
        presentation.labels > 0.01;
      projected.copy(n.group.position);
      projected.y += ((n.spec.size?.[1] ?? 0.6) / 2) * n.pose.scale;
      projected.project(camera);
      const x = (projected.x * 0.5 + 0.5) * width,
        y = (-projected.y * 0.5 + 0.5) * height;
      const w = n.label.offsetWidth || 130,
        h = n.label.offsetHeight || 44;
    ...
    for (const l of links) {
      const source = nodes.get(l.spec.from)!,
        dest = nodes.get(l.spec.to)!;
      const visibility = Math.min(source.pose.opacity, dest.pose.opacity);
      const start = anchor(source, dest.group.position),
        end = anchor(dest, source.group.position);
      const bend1 = start.clone().lerp(end, 0.42),
        bend2 = start.clone().lerp(end, 0.58);
      bend1.y += 0.2;
      bend2.y += 0.2;
      ...
      l.material.opacity =
        visibility *
        (currentActive.has(l.spec.id) ? 0.12 + 0.83 * presentation.flow : 0.12);
      l.line.visible = visibility > 0.05;
      l.arrow.visible = visibility > 0.25 && currentActive.has(l.spec.id);
      l.arrow.position.copy(
        end
          .clone()
          .sub(end.clone().sub(bend2).normalize().multiplyScalar(0.07)),
      );
      ...
      if (l.packet.visible) {
        const progress = isReduced
          ? 0.5
          : (clock * 0.27 + links.indexOf(l) * 0.19) % 1;
        l.packet.position.copy(pointAt(l.points, progress));
        (l.packet.material as THREE.MeshBasicMaterial).opacity =
          visibility *
          presentation.flow *
          Math.min(1, progress * 10, (1 - progress) * 10);
      }
    }
  ...
  function tick(time: number) {
    ...
    if (!paused && !isReduced)
      clock += Math.min((time - (lastTime || time)) / 1000, 0.05);
    lastTime = time;
    draw();
    if (
      (!paused && !isReduced && (stop.activeConnections?.length ?? 0) > 0) ||
      transition?.isActive()
    )
  ...
  function goTo(next: SlideStop, immediate = false) {
    transition?.kill();
    stop = next;
    const duration =
      immediate || isReduced ? 0 : (next.transition?.duration ?? 1.6);
    const destination = {
    ...
    // Fade annotations before movement, then bring them back at stable positions.
