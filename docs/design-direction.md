...
Use authored stacked drawings below 720px where available. Dense drawings without a narrow alternative keep a readable internal width with horizontal scrolling. Opening a canvas offers a wide drawing with zoom and scroll. Native touch scrolling is supported; custom pinch-to-zoom and free 3D orbit are not implemented. Check page overflow, label sizes, clipping, control reachability, focus, and reduced motion at 390px.
