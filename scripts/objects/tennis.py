...
stripe=material('Court stripe','386e5e')
for i in range(8):
    box('Mown court stripe',(0,-2.85+i*.81,.024),(3.25,.405,.003),stripe,.001)
for y in [-3.05,3.05]:path('Centre baseline tick',[(0,y,.037),(0,y-math.copysign(.12,y),.037)],.014,chalk)
...
seam=parent(path('Ball seam',[(.073*math.cos(a),.025*math.sin(a*2),.066*math.sin(a)) for a in [i*2*math.pi/40 for i in range(40)]],.003,shirt,True),ball)
seam.scale=(1/.073,)*3
...
scene.frame_set(0)
# Combine rigid detail by material under its existing animated parent. This retains
# independently animated joints and racket contact transforms, while reducing web draw calls.
from collections import defaultdict
batches=defaultdict(list)
for obj in list(scene.objects):
    if obj.type in {'MESH','CURVE'} and not obj.animation_data and not obj.children and len(obj.data.materials)==1:
        batches[(obj.parent,obj.data.materials[0])].append(obj)
depsgraph=bpy.context.evaluated_depsgraph_get()
for (parent_obj,mat),objects in batches.items():
    if len(objects)<2:continue
    verts=[];faces=[];smooth=[]
    for obj in objects:
        evaluated=obj.evaluated_get(depsgraph);part=evaluated.to_mesh()
        transform=obj.matrix_world if parent_obj is None else parent_obj.matrix_world.inverted()@obj.matrix_world
        offset=len(verts);verts.extend(tuple(transform@v.co) for v in part.vertices)
        faces.extend(tuple(offset+i for i in f.vertices) for f in part.polygons)
        smooth.extend(f.use_smooth for f in part.polygons);evaluated.to_mesh_clear()
    data=bpy.data.meshes.new('Batched '+mat.name);data.from_pydata(verts,[],faces);data.update();data.materials.append(mat)
    for face,use_smooth in zip(data.polygons,smooth):face.use_smooth=use_smooth
    combined=bpy.data.objects.new((parent_obj.name if parent_obj else 'Court')+' / '+mat.name,data)
    bpy.context.collection.objects.link(combined);combined.parent=parent_obj
...
scene.render.engine='CYCLES';scene.cycles.samples=20;scene.cycles.use_denoising=True;scene.render.resolution_x=960;scene.render.resolution_y=960;scene.render.resolution_percentage=100;scene.render.film_transparent=True
