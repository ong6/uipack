...
scene=bpy.context.scene; scene.render.fps=30; scene.frame_start=0; scene.frame_end=162
...
# Thin beveled court plinth, linework and a real mesh net.
box('Court base',(0,.42,-.075),(4.28,4.4,.15),border,.09)
box('Court playing surface',(0,.42,.01),(3.92,4.05,.025),courtmat,.04)
for x in [-1.63,1.63]:path('Sideline',[(x,-1.38,.03),(x,2.18,.03)],.012,chalk)
for y in [-1.38,.48,2.18]:path('Baseline', [(-1.63,y,.03),(1.63,y,.03)],.012,chalk)
path('Service divider',[(0,.48,.03),(0,2.18,.03)],.01,chalk)
for x in [-1.83,1.83]:box('Net post',(x,1.30,.45),(.065,.065,.87),rubber,.016)
for i in range(27):
    x=-1.8+i*3.6/26;h=.76+.035*(abs(x)/1.8)**2
    path('Net vertical',[(x,1.3,.15),(x,1.3,h)],.004,string)
for i in range(6):path('Net horizontal',[(-1.8,1.3,.16+i*.105),(1.8,1.3,.16+i*.105)],.004,string)
path('Net tape',[(-1.84+i*3.68/30,1.3,.76+.035*(abs(-1.84+i*3.68/30)/1.84)**2) for i in range(31)],.021,shirt)
...
def pose(t):
    ...
    root=Vector((.16*weight,-.63,1.24-.085*weight+.045*math.sin(math.pi*max(0,min(1,t/.55)))))
    ...
    direction=interp((.60,.02,.55),(.9,-.20,.10),load).lerp(Vector((.95,.10,.30)),strike).lerp(Vector((-.60,.15,.60)),follow).lerp(Vector((.50,.07,.62)),recover).normalized()
    racket.rotation_quaternion=Vector((0,0,1)).rotation_difference(direction)
    bpy.context.view_layer.update()
    return racket.matrix_world@Vector((0,0,.63))
contact_time=1.9;contact=pose(contact_time)
animated=[body,hips,racket,ball,*parts.values(),*joints.values(),*shoes.values()]
for frame in range(0,163,2):
    t=frame/30;pose(t)
    if t<=contact_time:
        q=max(0,min(1,(t-.15)/(contact_time-.15)))
        ball.location=interp((-.55,2.13,.95),contact,q);ball.location.z+=.38*math.sin(math.pi*q)
    else:
        q=max(0,min(1,(t-contact_time)/.70));ball.location=contact.lerp(Vector((-.8,2.12,.18)),q);ball.location.z+=.48*math.sin(math.pi*q)
    for o in animated:key(o,frame)
# Sample exact impact (frame 57) so ball and racket remain joined in exported animation.
pose(contact_time);ball.location=contact
for o in animated:key(o,57)
for o in animated:
    if o.animation_data and o.animation_data.action:o.animation_data.action.name='Return · '+o.name
scene.frame_set(0)
# Export just meshes/curves and animation, with one synchronized scene clip.
...
(ROOT/'src/objects/tennis-asset.ts').write_text('// Generated by scripts/objects/tennis.py. Original UI Pack asset.\nexport default "'+base64.b64encode(data).decode()+'";\n')
...
bpy.ops.object.camera_add(location=(3.6,-8,6));cam=bpy.context.object;cam.rotation_euler=(Vector((0,.15,1.1))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='ORTHO';cam.data.ortho_scale=5.8;scene.camera=cam
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'tennis-return.blend'))
for frame in [0,45,57,72]:
    scene.frame_set(frame);scene.render.filepath=str(OUT/f'pose-{frame}.png');bpy.ops.render.render(write_still=True)
(OUT/'verification.json').write_text(json.dumps({'bytes':len(data),'contact_frame':57,'fps':30,'frames':163,'source':'Original UI Pack model; no external assets'},indent=2))
