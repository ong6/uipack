import { Connector, Defs, Figure, Group, Label, Lane, Node, Packet, anchor, route, type Point } from "../src";
...
export function HabitatWide() {
  ...
  const busX = 232; // vertical bus between clients and platform
  const storeBusX = 1000;
  const cdc = { x: 528, y: 384, w: 200, h: 56 };

  const clientPaths: Point[][] = CLIENTS.map((_, i) => {
  ...
  const sinkPaths: Point[][] = SINKS.map((_, i) => {
    ...
    <>
      ...
      <Group {...platform} title="Habitat">
        {PLATFORM.map(([label, sub], i) => (
          <Node key={label} x={platform.x + 24 + (i % 3) * 192} y={platform.y + 44 + Math.floor(i / 3) * 72} w={168} h={56} label={label} sub={sub} align="left" />
        ))}
      ...
      <Label x={platform.x + platform.w / 2 + 10} y={platform.y + platform.h + 28} text="changes" accent={false} />
    </>
  );
}
